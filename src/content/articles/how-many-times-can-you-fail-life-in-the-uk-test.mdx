...
| Cost per attempt | £50 |
| Minimum wait between retakes | [VERIFY: check GOV.UK for minimum waiting period] |
...
⚠️ There is a minimum waiting period between attempts — [VERIFY: confirm exact period from GOV.UK]  
...
There is a minimum waiting period between test attempts. [VERIFY: confirm the exact minimum waiting period from GOV.UK — previously 7 days, confirm current rule]
...
[VERIFY: check whether the Home Office or test provider publishes average attempt data. If not available, flag as not officially published rather than estimating.]

Most people who prepare thoroughly pass on their first attempt. Among those who fail once, the majority pass on the second attempt after targeted revision. Failing twice or more is less common but does happen — usually when the revision approach has not changed between attempts.
...
[VERIFY: confirm minimum waiting period from GOV.UK. State the confirmed period once verified.]
...
Not immediately. There is a minimum waiting period between attempts. [VERIFY: confirm the exact period from GOV.UK.] Use the waiting time productively to revise your weak areas.
